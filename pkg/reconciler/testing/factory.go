...
import (
	...
	fakev2client "knative.dev/net-ingressv2/pkg/client/injection/client/fake"
...
func MakeFactory(ctor Ctor) rtesting.Factory {
	return func(t *testing.T, r *rtesting.TableRow) (
		...
		ctx, v2client := fakev2client.With(ctx, ls.GetIngressV2Objects()...)
		...
		rtesting.PrependGenerateNameReactor(&v2client.Fake)
		...
		for _, reactor := range r.WithReactors {
			...
			v2client.PrependReactor("*", "*", reactor)
		}
		...
		actionRecorderList := rtesting.ActionRecorderList{client, kubeclient, v2client}
